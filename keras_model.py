...
def res_conv18(x, s, filters):
    ...
    f1, f2 = filters
    # Layer 1
    x = Conv2D(f1, kernel_size=(3,3), padding = 'same', strides = (1,s))(x)
    print("what",x)
    x = BatchNormalization()(x)#axis=1 put in comments to load resnet202_conformer
    x = Activation('relu')(x)
    # Layer 2
    x = Conv2D(f2, kernel_size=(3,3), padding = 'same')(x)
    x = BatchNormalization()(x)#axis=1 put in comments to load resnet202_conformer
    # Processing Residue with conv(1,1)
    x_skip = Conv2D(f2, kernel_size=(1,1), strides = (1,s))(x_skip)
    # Add Residue
    x = Add()([x, x_skip])     
    x = Activation('relu')(x)
    return x
...
def resnet_2020(t_pool_size,f_pool_size, spec_cnn, nb_cnn2d_filt):
    ...
    # 3rd stage
    print("\n############ STAGE 2 ##############\n")
    spec_cnn = res_conv18(spec_cnn, s=1, filters=(nb_cnn2d_filt*2, nb_cnn2d_filt))
    print(spec_cnn)
    spec_cnn = res_identity18(spec_cnn, filters=(nb_cnn2d_filt*2,nb_cnn2d_filt))
    print(spec_cnn)
...
def resnet34(t_pool_size,f_pool_size, spec_cnn, nb_cnn2d_filt):
    ...
    # 4th stage
    print("\n############ STAGE 3 ##############\n")
    spec_cnn = res_conv18(spec_cnn, s=1, filters=(nb_cnn2d_filt*4, nb_cnn2d_filt*4))
    print(spec_cnn)
    ...
    # 5th stage
    print("\n############ STAGE 4 ##############\n")
    spec_cnn = res_conv18(spec_cnn, s=1, filters=(nb_cnn2d_filt*8, nb_cnn2d_filt*8))
    print(spec_cnn)
...
def resnet18(input_im):
    ...
    x = Dropout(0.2)(x)
    #17/5/2022 put dense layers in comments
    #x = Dense(512, activation = 'relu')(x)
    print(x)
    #x = Dense(12, activation = 'sigmoid')(x) ## 12 = nb_classes(the total number of sound events), was softmax but this is a multi label problem
...
def get_model(data_in, data_out, dropout_rate, nb_cnn2d_filt, f_pool_size, t_pool_size,
              ...
              model_approach, 
              dconv_kernel_size,
              nb_conf, depth, decoder, simple_parallel): ####### CUSTOM CODE
    ...
    if model_approach == 1 or model_approach == 2:# or model_approach == 6:
        #15/5/2022 need this for ensemble to load correctly
        keras.backend.set_image_data_format('channels_first')
        # 1st stage
        # here we perform maxpooling, see the figure above
        x = spec_cnn
        ...
        elif model_approach == 2 or model_approach == 6:
            spec_cnn = resnet34(t_pool_size,f_pool_size,spec_cnn, nb_cnn2d_filt)
        #added maxpool for reshaping output
        #spec_cnn = Permute((2, 1, 3))(spec_cnn)
        print('after resnet34', spec_cnn)
        spec_cnn = Permute((2, 1, 3))(spec_cnn)
        print(spec_cnn)
        
        spec_cnn = AveragePooling2D(pool_size=(4,6), padding='same')(spec_cnn)
        spec_cnn = Reshape((spec_cnn.shape[1], spec_cnn.shape[-2]*spec_cnn.shape[-1]))(spec_cnn)
        ##need to pool to bring sequence to (60,64,2) dimension(seq-len, mel-bands, idk(??))
        spec_cnn = Dense(512, activation = 'relu')(spec_cnn)
        print(spec_cnn)
        #the below dense 12 filter layer was used in pseudoresnet34 
        spec_cnn = Dense(12, activation = 'sigmoid')(spec_cnn) ## 12 = nb_classes(the total number of sound events), wasa softmax but this is multilabel problem
    ...
    if model_approach == 3:
        ...
        #12/5/2022 commented below cnn to use resnet-conformer
        keras.backend.set_image_data_format('channels_first')
        spec_cnn = Conv2D(filters=nb_cnn2d_filt, kernel_size=(3, 3), padding='same')(spec_cnn)
        print("(64, 300, 64) ",spec_cnn)
        spec_cnn = BatchNormalization()(spec_cnn)
        ...
        #12/5/2022 conv layers from zhang, i didnt use these during all my working tests, i used the above layers
        '''
        conv_size = [nb_cnn2d_filt, 2*nb_cnn2d_filt, 4*nb_cnn2d_filt]
        for i in range(3):
            keras.backend.set_image_data_format('channels_first')
        ...
        #(60, 128)
        ###### DENSE LAYERS #########
        #13/5/2022 commented these (resnet34_conformer_gru_16attn3)
        keras.backend.set_image_data_format('channels_first')
        spec_cnn = TimeDistributed(Dense(256, activation = 'relu',kernel_regularizer=keras.regularizers.l1_l2(l1=0, l2=0.001)))(spec_cnn)
        spec_cnn = TimeDistributed(Dense(128, activation = 'relu',kernel_regularizer=keras.regularizers.l1_l2(l1=0, l2=0.001)))(spec_cnn)
        spec_cnn = TimeDistributed(Dense(36, activation = 'tanh',kernel_regularizer=keras.regularizers.l1_l2(l1=0, l2=0.001)))(spec_cnn)
    ...
    for nb_rnn_filt in rnn_size:
        if decoder == 0 and model_approach != 3:
            ...
            print('bidirect') 
        elif decoder == 1 and model_approach != 3:
            keras.backend.set_image_data_format('channels_last')
            spec_rnn = Bidirectional(
    ...
    if simple_parallel == True:
        x_output = TimeDistributed(Dense(data_out[-1], kernel_initializer=keras.initializers.GlorotUniform(seed=None),kernel_regularizer=keras.regularizers.l1_l2(l1=0, l2=0.001)))(doa)
        x_output = Activation('relu')(x_output)
        x_output = Dropout(dropout_rate)(x_output)
        x_output = TimeDistributed(Dense(12, kernel_initializer=keras.initializers.GlorotUniform(seed=None),kernel_regularizer=keras.regularizers.l1_l2(l1=0, l2=0.001)))(x_output)
        x_output = Activation('tanh', name='doa_out_x')(x_output)
        x_output = Dropout(dropout_rate)(x_output)

        y_output = TimeDistributed(Dense(data_out[-1], kernel_initializer=keras.initializers.GlorotUniform(seed=None),kernel_regularizer=keras.regularizers.l1_l2(l1=0, l2=0.001)))(doa)
        y_output = Activation('relu')(y_output)
        y_output = Dropout(dropout_rate)(y_output)
        y_output = TimeDistributed(Dense(12, kernel_initializer=keras.initializers.GlorotUniform(seed=None),kernel_regularizer=keras.regularizers.l1_l2(l1=0, l2=0.001)))(y_output)
        y_output = Activation('tanh', name='doa_out_y')(y_output)
        y_output = Dropout(dropout_rate)(y_output)
        z_output = TimeDistributed(Dense(data_out[-1], kernel_initializer=keras.initializers.GlorotUniform(seed=None),kernel_regularizer=keras.regularizers.l1_l2(l1=0, l2=0.001)))(doa)
        z_output = Activation('relu')(z_output)
        z_output = Dropout(dropout_rate)(z_output)
        z_output = TimeDistributed(Dense(12, kernel_initializer=keras.initializers.GlorotUniform(seed=None),kernel_regularizer=keras.regularizers.l1_l2(l1=0, l2=0.001)))(z_output)
    ...
    if is_accdoa:
        ...
        model.compile(optimizer=Adam(learning_rate=0.001), loss='mse', metrics=['accuracy'])#0.0001
    else:
        # FC - SED
        sed = spec_rnn
        for nb_fnn_filt in fnn_size:
