...
def resnet_2020(t_pool_size,f_pool_size, spec_cnn, nb_cnn2d_filt):
    ...
    spec_cnn = Conv2D(nb_cnn2d_filt, kernel_size=(1,1), strides=(1, 1), padding='same', kernel_regularizer=tensorflow.keras.regularizers.l2(0.001))(spec_cnn)
...
def get_model(data_in, data_out, dropout_rate, nb_cnn2d_filt, f_pool_size, t_pool_size,
    ...
    if model_approach == 3:
        ...
        '''
        for i in range(3):
            spec_cnn = Conv2D(nb_cnn2d_filt*(i+1), kernel_size=(3, 3), padding='same')(spec_cnn)
            spec_cnn = BatchNormalization(axis=1)(spec_cnn)
            spec_cnn = Activation('relu')(spec_cnn)
            spec_cnn = MaxPooling2D(pool_size=(t_pool_size[i],f_pool_size[i]))(spec_cnn)
        #(256, 60, 2)
        ...
        resnet_2020(t_pool_size=t_pool_size, f_pool_size=f_pool_size, spec_cnn=spec_cnn, nb_cnn2d_filt=nb_cnn2d_filt)
        spec_cnn = TimeDistributed(Dense(128, activation = 'relu',kernel_regularizer=keras.regularizers.l1_l2(l1=0, l2=0.001)))(spec_cnn)
        spec_cnn = TimeDistributed(Dense(36, activation = 'tanh',kernel_regularizer=keras.regularizers.l1_l2(l1=0, l2=0.001)))(spec_cnn)
    ...
    for nb_rnn_filt in rnn_size:
        ...
        #Zhang decoder, conformer
            ...
            spec_cnn = TimeDistributed(Dense(256, activation = 'relu',kernel_regularizer=keras.regularizers.l1_l2(l1=0, l2=0.001)))(spec_cnn)
            spec_cnn = TimeDistributed(Dense(128, activation = 'relu',kernel_regularizer=keras.regularizers.l1_l2(l1=0, l2=0.001)))(spec_cnn)
        elif decoder == 0 and model_approach != 3 and model_approach != 6:
            keras.backend.set_image_data_format('channels_last')
            spec_rnn = Bidirectional(
                GRU(nb_rnn_filt, activation='tanh', dropout=dropout_rate, recurrent_dropout=dropout_rate,
                    return_sequences=True),
            ...
            print('bidirect') 
        elif decoder == 1 and model_approach != 3 and model_approach != 6:
            keras.backend.set_image_data_format('channels_last')
            spec_rnn = Bidirectional(
