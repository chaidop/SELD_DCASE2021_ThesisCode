...
def Attention(dim, inputs, heads=8, dim_head=64, dropout=0.0, max_pos_emb=512, context=None, mask=None, context_mask=None):
    inner_dim = dim_head * heads
    sheads = heads
    sscale = dim_head ** -0.5

    sto_q = tf.keras.layers.Dense(inner_dim, use_bias=False)
    sto_kv = tf.keras.layers.Dense(inner_dim * 2, use_bias=False)
    sto_out = tf.keras.layers.Dense(dim)

    smax_pos_emb = max_pos_emb
    srel_pos_emb = tf.keras.layers.Embedding(2 * max_pos_emb + 1, dim_head)

    sdropout = tf.keras.layers.Dropout(dropout)
